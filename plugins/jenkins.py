...
def jenkins_run(message, job):
    ...
    if resp.status_code == 201:
        message.reply("Jenkins job '{}' started. Check Slack or {} for results.".format(job, job_url))
    else:
...
def jenkins_check(message, job):
    ...
    if (resp.status_code == 200):
       ...
       message.reply("Jenkins job '{}' is marked '{}'. Check {} for information on the last build.".format( \
           job, status, last_build_url))
    else:
